...
namespace App\Myclasses\Customclasses;
/**
...
class TMDb
{
	...
	public function getAvailableImageSizes($imagetype)
	{
		...
		{
			 new TMDbException('No configuration available to retrieve available image sizes');
		}
	}
	...
	private function _makeCall($function, $params = NULL, $session_id = NULL, $method = TMDb::GET)
	{
		...
		{
			 new TMDbException('CURL-extension not loaded');
		}
		...
		{
			 new TMDbException('Server error on "'.$url.'": '.$response);
		}
	}
